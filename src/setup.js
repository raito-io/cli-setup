...
async function run() {
  try {
    ...
    core.info(
      `Finding assets for Raito CLI version ${version} on ${platform}/${arch}.`
    )

    const url = `https://github.com/raito-io/cli/releases/download/${version}/raito-${version.slice(1)}-${platform}_${arch}.tar.gz`

    // Download requested version
    const auth = githubToken ? `token ${githubToken}` : null
